...
sed 's/-march=i686/-march=x86-64/g' $source/system/base/glibc/actions.py > $destination/system/base/glibc/actions.py
sed -e 's/<Path fileType="library">\/lib<\/Path>/ <Path fileType="library">\/lib<\/Path>\n             <Path fileType="library">\/lib64<\/Path>/g' -e 's/<Path fileType="library">\/usr\/lib\/<\/Path>/ <Path fileType="library">\/usr\/lib\/<\/Path>\n             <Path fileType="library">\/usr\/lib64\/<\/Path>/g' $source/system/base/glibc/pspec.xml > $destination/system/base/glibc/pspec.xml

#fix for python
echo "pytho x86-64 için uygun hale getiriliyor"
sed 's/--enable-unicode=ucs4/--enable-unicode=ucs4 \\\n                         --enable-universalsdk \\\n                         --with-universal-archs=64-bit /g' $source/system/base/python/actions.py > $destination/system/base/python/actions.py

#fix for perl
echo "perl x86-64 için uygun hale getiriliyor"
sed 's/i686/x86_64/g' $source/system/base/perl/actions.py > $destination/system/base/perl/actions.py

#fix for baselayout
echo "baselayout x86-64 için uygun hale getiriliyor"
sed 's/<Path fileType="library" permanent="true">\/lib<\/Path>/<Path fileType="library" permanent="true">\/lib<\/Path>\n            <Path fileType="library" permanent="true">\/lib64<\/Path>/' $source/system/base/baselayout/pspec.xml > $destination/system/base/baselayout/pspec.xml
sed 's/pisitools\.dosym("share\/man", "\/usr\/local\/man")/pisitools\.dosym("share\/man", "\/usr\/local\/man")\n    pisitools\.dosym("\/lib", "\/lib64")\n    pisitools\.dosym("\/usr\/lib", "\/usr\/lib64")/' $source/system/base/baselayout/actions.py > $destination/system/base/baselayout/actions.py
...
grep -v KLIBCARCH $source/system/devel/klibc/actions.py > $destination/system/devel/klibc/actions.py

#fix for gmp
echo "gmp x86-64 için uygun hale getiriliyor"
grep -v 'enable-fat' $source/system/devel/gmp/actions.py > $destination/system/devel/gmp/actions.py
...
sed 's/shelltools\.move("Makefile\.def", "Makefile")/shelltools\.move("Makefile\.def", "Makefile") \n    pisitools\.dosed("Makefile","options= ","options= \$(CFLAGS) -DNOFUNCDEF -DUTIME_H \$(LDFLAGS) ")/g' $source/system/base/ncompress/actions.py > $destination/system/base/ncompress/actions.py

#fix for lame
echo "lame x86-64 için uygun hale getiriliyor"
sed -e 's/shelltools\.makedirs("libmp3lame\/i386\/\.libs")/shelltools\.makedirs("libmp3lame\/%s\/\.libs" %(get\.HOST()))/g' -e 's/j1/j4/g' $source/multimedia/sound/lame/actions.py > $destination/multimedia/sound/lame/actions.py

#fix for ffmpeg
echo "ffmpeg x86-64 için uygun hale getiriliyor"
grep -v i686 $source/multimedia/video/ffmpeg/actions.py|grep -v enable-vaapi |sed 's/                            --prefix=\/usr/    autotools\.rawConfigure("--prefix=\/usr/g' > $destination/multimedia/video/ffmpeg/actions.py

#fix for sdl-gfx
echo "sdl-gfx x86-64 için uygun hale getiriliyor"
sed 's/--enable-mmx/--disable-mmx/g' $source/multimedia/library/sdl-gfx/actions.py > $destination/multimedia/library/sdl-gfx/actions.py

#fix for p7zip·
echo "p7zip x86-64 için uygun hale getiriliyor"
sed -e 's/from pisi\.actionsapi import pisitools/from pisi\.actionsapi import pisitools\nfrom pisi\.actionsapi import shelltools/g' -e 's/def build():/def build():\n    shelltools.copy("makefile\.linux_amd64", "makefile\.machine")/g' $source/util/archive/p7zip/actions.py > $destination/util/archive/p7zip/actions.py
...
sed 's/def build():/def build(): \n    shelltools\.export("USE_64","1")/g' $source/network/library/nss/actions.py > $destination/network/library/nss/actions.py

#fix for lapack
echo "lapack x86-64 için uygun hale getiriliyor."
sed 's/pisitools\.dosed("make.inc", "-O2", "%s -fPIC -funroll-all-loops" % get.CFLAGS())/pisitools.dosed("make.inc", "-O2", "%s -fPIC -m64 -funroll-all-loops" % get.CFLAGS())\n    pisitools\.dosed("make.inc", "NOOPT    =", "NOOPT    =-m64 -fPIC ")/g' $source/science/library/lapack/actions.py > $destination/science/library/lapack/actions.py

#fix for imlib2
echo "imlib2 x86-64 için uygun hale getiriliyor."
sed 's/--disable-amd64/--enable-amd64/g' $source/multimedia/library/imlib2/actions.py > $destination/multimedia/library/imlib2/actions.py

#fix for libsdl
echo "libsdl x86-64 için uygun hale getiriliyor."
sed 's/--enable-nasm/--disable-nasm/g' $source/multimedia/library/libsdl/actions.py > $destination/multimedia/library/libsdl/actions.py

#fix for atlas
echo "atlas x86-64 için uygun hale getiriliyor."
sed 's/-b 32/-b 64/g' $source/multimedia/library/atlas/actions.py > $destination/multimedia/library/atlas/actions.py

#fix for qca
echo "qca x86-64 için uygun hale getiriliyor."
sed 's/get\.CXXFLAGS()/get\.CXXFLAGS() + " -fPIC "/g' $source/programming/library/qca/actions.py > $destination/programming/library/qca/actions.py

#fix for libstdc++
echo "libstdc++ x86-64 için uygun hale getiriliyor."
sed 's/-mcpu=i686/-mcpu=x86-64/g' $source/programming/library/libstdc++/actions.py > $destination/programming/library/libstdc++/actions.py
...
sed -e 's/\/usr\/lib\/libjs\.a/\/usr\/lib64\/libjs\.a/g' -e 's/"\/usr\/lib\/libjs\.so", "libjs\.so\.1"/"\/usr\/lib64\/libjs\.so", "libjs\.so\.1"/g' -e 's/"\/usr\/lib\/libjs\.so\.1", "\/usr\/lib\/libjs\.so"/"\/usr\/lib64\/libjs\.so\.1", "\/usr\/lib\/libjs\.so"/g' $source/programming/library/spidermonkey/actions.py > $destination/programming/library/spidermonkey/actions.py
sed 's/<Path fileType="library">\/usr\/lib<\/Path>/ <Path fileType="library">\/usr\/lib<\/Path> \n             <Path fileType="library">\/usr\/lib64<\/Path>/g' $source/programming/library/spidermonkey/pspec.xml > $destination/programming/library/spidermonkey/pspec.xml

#fix for perlconsole
echo "perlconsole x86-64 için uygun hale getiriliyor."
sed 's/i686/x86_64/g' $source/programming/language/perl/perlconsole/actions.py > $destination/programming/language/perl/perlconsole/actions.py

#fix for git
echo "git x86-64 için uygun hale getiriliyor."
sed 's/i686/x86_64/g' $source/programming/vcs/git/actions.py > $destination/programming/vcs/git/actions.py

#fix for subversion
echo "subversion x86_64 için uygun hale getiriliyor."
sed 's/i686/x86_64/' $source/programming/vcs/subversion/actions.py > $destination/programming/vcs/subversion/actions.py
sed 's/i686/x86_64/' $source/programming/vcs/subversion/pspec.xml > $destination/programming/vcs/subversion/pspec.xml
...
sed 's/i686/x86_64/g' $source/multimedia/graphics/imagemagick/actions.py > $destination/multimedia/graphics/imagemagick/actions.py
...
sed 's/i686/x86_64/g' $source/programming/language/perl/perl-Digest-HMAC/actions.py > $destination/programming/language/perl/perl-Digest-HMAC/actions.py

#fix for perl-Carp-Clan
echo "perl-Carp-Clan x86-64 için uygun hale getiriliyor"
sed 's/i686/x86_64/g' $source/programming/language/perl/perl-Carp-Clan/actions.py > $destination/programming/language/perl/perl-Carp-Clan/actions.py

#fix for perl-Crypt-PasswdMD5
echo "perl-Crypt-PasswdMD5 x86-64 için uygun hale getiriliyor"
sed 's/i686/x86_64/g' $source/programming/language/perl/perl-Crypt-PasswdMD5/actions.py > $destination/programming/language/perl/perl-Crypt-PasswdMD5/actions.py

#fix for perl-Convert-ASN1
echo "perl-Convert-ASN1 x86-64 için uygun hale getiriliyor"
sed 's/i686/x86_64/g' $source/programming/language/perl/perl-Convert-ASN1/actions.py > $destination/programming/language/perl/perl-Convert-ASN1/actions.py

#fix for perl-Crypt-Simple
echo "perl-Crypt-Simple x86-64 için uygun hale getiriliyor"
sed 's/i686/x86_64/g' $source/programming/language/perl/perl-Crypt-Simple/actions.py > $destination/programming/language/perl/perl-Crypt-Simple/actions.py

#fix for perl-Crypt-SmbHash
echo "perl-Crypt-SmbHash x86-64 için uygun hale getiriliyor"
sed 's/i686/x86_64/g' $source/programming/language/perl/perl-Crypt-SmbHash/actions.py > $destination/programming/language/perl/perl-Crypt-SmbHash/actions.py

#fix for perl-Date-Manip
echo "perl-Date-Manip x86-64 için uygun hale getiriliyor"
sed 's/i686/x86_64/g' $source/programming/language/perl/perl-Date-Manip/actions.py > $destination/programming/language/perl/perl-Date-Manip/actions.py

#fix for perl-Date-Calc
echo "perl-Date-Calc x86-64 için uygun hale getiriliyor"
sed 's/i686/x86_64/g' $source/programming/language/perl/perl-Date-Calc/actions.py > $destination/programming/language/perl/perl-Date-Calc/actions.py

#fix for perl-Email-Abstract
echo "perl-Email-Abstract x86-64 için uygun hale getiriliyor"
sed 's/i686/x86_64/g' $source/programming/language/perl/perl-Email-Abstract/actions.py > $destination/programming/language/perl/perl-Email-Abstract/actions.py

#fix for perl-Email-Address
echo "perl-Email-Address x86-64 için uygun hale getiriliyor"
sed 's/i686/x86_64/g' $source/programming/language/perl/perl-Email-Address/actions.py > $destination/programming/language/perl/perl-Email-Address/actions.py

#fix for perl-Email-Date-Format
echo "perl-Email-Date-Format x86-64 için uygun hale getiriliyor"
sed 's/i686/x86_64/g' $source/programming/language/perl/perl-Email-Date-Format/actions.py > $destination/programming/language/perl/perl-Email-Date-Format/actions.py

#fix for perl-Email-Date
echo "perl-Email-Date x86-64 için uygun hale getiriliyor"
sed 's/i686/x86_64/g' $source/programming/language/perl/perl-Email-Date/actions.py > $destination/programming/language/perl/perl-Email-Date/actions.py

#fix for perl-Email-MessageID
echo "perl-Email-MessageID x86-64 için uygun hale getiriliyor"
sed 's/i686/x86_64/g' $source/programming/language/perl/perl-Email-MessageID/actions.py > $destination/programming/language/perl/perl-Email-MessageID/actions.py

#fix for pam
echo "pam x86-64 için uygun hale getiriliyor"
sed 's/<Path fileType="library">\/usr\/lib<\/Path>/ <Path fileType="library">\/usr\/lib<\/Path> \n             <Path fileType="library">\/usr\/lib64<\/Path>/g' $source/system/base/pam/pspec.xml > $destination/system/base/pam/pspec.xml
...
sed 's/for chost in \["", "i686-pc-linux-gnu-"\]:/host= get\.HOST()+"-"\n    for chost in \["", host\]:/g' $source/system/devel/icecream/actions.py > $destination/system/devel/icecream/actions.py
...
sed 's/--with-cpu=sse/--with-cpu=x86-64/' $source/multimedia/sound/mpg123/actions.py > $destination/multimedia/sound/mpg123/actions.py

#fix for squashfs-tools
echo "squashfs-tools x86-64 için uygun hale getiriliyor"
sed 's/def build():/def setup():\n    for f in \["LZMA\/C\/LzFind\.o", "LZMA\/C\/LzmaDec\.o", "LZMA\/C\/LzmaEnc\.o", "LZMA\/C\/LzmaLib\.o", "LZMA\/C\/Alloc\.o"\]:\n        shelltools\.unlink(f)\n\ndef build():/g' $source/hardware/disk/squashfs-tools/actions.py > $destination/hardware/disk/squashfs-tools/actions.py

#fix for obexftp
echo "obexftp x86-64 için uygun hale getiriliyor"
sed 's/i686/x86_64/g' $source/hardware/library/obexftp/actions.py > $destination/hardware/library/obexftp/actions.py
...
sed 's/<\/Patches>/    <Patch>corporate-x86_64\.patch<\/Patch>\n        <\/Patches>/g' $source/system/base/comar/pspec.xml > $destination/system/base/comar/pspec.xml
...
sed 's/pisitools\.remove("\/usr\/lib\//pisitools\.remove("\/usr\/lib64\//g' $source/server/database/firebird/actions.py > $destination/server/database/firebird/actions.py

#fix for libnut
echo "libnut x86-64 için uygun hale getiriliyor"
cp files/libnut_fPIC.patch $destination/multimedia/converter/libnut/files/
sed 's/        <\/Patches>/            <Patch level="1">libnut_fPIC\.patch<\/Patch>\n        <\/Patches>/g' $source/multimedia/converter/libnut/pspec.xml > $destination/multimedia/converter/libnut/pspec.xml
...
sed 's/<Patch level="1">pilot-link-0\.12\.3-md5\.patch<\/Patch>/<!--Patch level="1">pilot-link-0\.12\.3-md5\.patch<\/Patch-->\n            <Patch level="1">pilot-link-0\.12\.3-int_types\.patch<\/Patch>/g' $source/hardware/mobile/pilot-link/pspec.xml > $destination/hardware/mobile/pilot-link/pspec.xml
sed 's/i686-linux-thread-multi/x86_64-linux-thread-multi/g' $source/hardware/mobile/pilot-link/actions.py > $destination/hardware/mobile/pilot-link/actions.py
...
sed 's/        <\/Patches>/            <Patch level="1">v4l_compat_ioctl32\.patch<\/Patch>\n        <\/Patches>/g' $source/kernel/default/drivers/module-syntekdriver/pspec.xml > $destination/kernel/default/drivers/module-syntekdriver/pspec.xml
...
sed 's/        <\/Patches>/            <Patch level="1">0001-Fix-bug-when-compiling-on-2\.6\.29-or-greater-64bit-ke\.patch<\/Patch>\n        <\/Patches>/g' $source/kernel/default/drivers/module-r5u870/pspec.xml > $destination/kernel/default/drivers/module-r5u870/pspec.xml
...
sed 's/<Archive sha1sum="6e5bd89fbd41358e2d19a80c87aab73dee983523" type="tarbz2">http://cekirdek.pardus.org.tr/~fatih/dist/nvidia/NVIDIA-Linux-x86-190.53.tar.bz2</Archive>/<Archive sha1sum="ea021fe181b2d86d97ee886dd2176c731cd5a565" type="tarbz2">http://members.comu.edu.tr/nyucel/source/NVIDIA-Linux-x86_64-190.42.tar.bz2</Archive>/' $source/kernel/default/drivers/module-nvidia-current/pspec.xml > $destination/kernel/default/drivers/module-nvidia-current/pspec.xml
...
#fix for pam
echo "pam x86-64 için uygun hale getiriliyor"
sed 's/<Path fileType="library">\/lib<\/Path>/ <Path fileType="library">\/lib<\/Path> \n             <Path fileType="library">\/lib64<\/Path>/g' $source/system/base/pam/pspec.xml > $destination/system/base/pam/pspec.xml
...
sed 's/i686-pc-linux-gnu/x86_64-pc-linux-gnu-/' $source/system/devel/icecream/actions.py > $destination/system/devel/icecream/actions.py
